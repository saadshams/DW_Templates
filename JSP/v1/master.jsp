...
PreparedStatement StatementrsCity = ConnrsCity.prepareStatement("SELECT *  FROM world.city");
...
<table align="center" border="1">
  <tr> 
    <td align="default" width="33%"> ID </td>
    <td align="default" width="33%"> Name </td>
    <td align="default" width="33%"> CountryCode </td>
  </tr>
  <% while ((rsCity_hasData)&&(Repeat1__numRows-- != 0)) { %>
  <tr> 
    <td align="default" width="33%"> 
      <a href="detail.jsp?<%= MM_keepBoth + ((MM_keepBoth!="")?"&":"") + "ID=" + (((rsCity_data = rsCity.getObject("ID"))==null || rsCity.wasNull())?"":rsCity_data) %>"><%=(((rsCity_data /* = rsCity.getObject("ID")*/)==null /* || rsCity.wasNull() */ )?"":rsCity_data)%></a> </td>
    <td align="default" width="33%"> 
      <%=(((rsCity_data = rsCity.getObject("Name"))==null || rsCity.wasNull())?"":rsCity_data)%> </td>
    <td align="default" width="33%"> 
      <%=(((rsCity_data = rsCity.getObject("CountryCode"))==null || rsCity.wasNull())?"":rsCity_data)%> </td>
  </tr>
  <%
  Repeat1__index++;
  rsCity_hasData = rsCity.next();
...
Records <%=(rsCity_first)%> to <%=(rsCity_last)%> of <%=(rsCity_total)%> 
</body>
</html>
<%
rsCity.close();
